<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="Invoice2.aspx.cs" Inherits="OffTheGrit1.Invoice2" %>

<asp:Content ID="Content1" ContentPlaceHolderID="title" runat="server">
    Invoice
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="head" runat="server">
    <style>
        .invoice-gradient {
            background: linear-gradient(145deg, hsl(0, 0%, 100%), hsl(39, 100%, 99%));
        }
        .dark .invoice-gradient {
            background: linear-gradient(145deg, hsl(220, 13%, 15%), hsl(220, 13%, 12%));
        }
        .header-gradient {
            background: linear-gradient(135deg, hsl(39, 100%, 50%), hsl(39, 100%, 45%));
        }
        .success-message {
            animation: slideInFromTop 0.6s ease-out;
        }
        @keyframes slideInFromTop {
            from {
                opacity: 0;
                transform: translateY(-30px);
            }
            to {
                opacity: 1;
                transform: translateY(0);
            }
        }
        .print-animation {
            transition: all 0.3s ease;
        }
        .print-animation:hover {
            transform: translateY(-2px);
            box-shadow: 0 8px 25px rgba(0, 0, 0, 0.15);
        }
        .dark .print-animation:hover {
            box-shadow: 0 8px 25px rgba(0, 0, 0, 0.3);
        }
        @media print {
            .no-print {
                display: none !important;
            }
            .invoice-container {
                box-shadow: none !important;
                margin: 0 !important;
                padding: 20px !important;
            }
        }
    </style>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    
    <!-- Invoice Section -->
    <section class="min-h-screen bg-gray-50 dark:bg-gray-900 py-12">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            
            <!-- Success Message (if applicable) -->
            <div id="successMessage" class="success-message mb-8 no-print">
                <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <svg class="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                        </div>
                        <div class="ml-4">
                            <h3 class="text-lg font-bold text-green-800 dark:text-green-300">Payment Successful!</h3>
                            <p class="text-green-700 dark:text-green-400 mt-1">Your order has been processed and your invoice is ready.</p>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Invoice Container -->
            <div class="invoice-gradient invoice-container rounded-2xl shadow-2xl overflow-hidden">
                
                <!-- Invoice Header -->
                <div class="header-gradient p-8 text-white">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                        <div class="flex items-center space-x-4">
                            <div class="w-16 h-16 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
                                <span class="text-2xl font-bold">OG</span>
                            </div>
                            <div>
                                <h1 class="text-3xl font-bold">OFF THE GRID</h1>
                                <p class="text-orange-100">Solar • Water • Security Solutions</p>
                            </div>
                        </div>
                        <div class="mt-4 md:mt-0 text-right">
                            <h2 class="text-2xl font-bold mb-2">INVOICE</h2>
                            <div class="bg-white bg-opacity-20 px-4 py-2 rounded-lg">
                                <p class="text-sm">Generated: <span id="currentDate"></span></p>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Invoice Content -->
                <div class="p-8">
                    
                    <!-- Billing & Shipping Information -->
                    <div class="grid md:grid-cols-2 gap-8 mb-8">
                        
                        <!-- Billing Information -->
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
                            <div class="flex items-center mb-4">
                                <div class="w-10 h-10 bg-blue-500 rounded-lg flex items-center justify-center mr-3">
                                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                    </svg>
                                </div>
                                <h4 class="text-lg font-bold text-gray-900 dark:text-white">Billing Information</h4>
                            </div>
                            <div class="space-y-2 text-gray-600 dark:text-gray-300">
                                <p class="font-medium text-gray-900 dark:text-white">
                                    <asp:Label ID="lblCustomerName" runat="server" />
                                </p>
                                <p>
                                    <asp:Label ID="lblbsafcdas" runat="server" />
                                </p>
                                <p class="flex items-center">
                                    <%--<svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"/>
                                    </svg>--%>
                                    <asp:Label ID="lblBillingEmail" runat="server" />
                                </p>
                            </div>
                        </div>

                        <!-- Shipping Information -->
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
                            <div class="flex items-center mb-4">
                                <div class="w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center mr-3">
                                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                                    </svg>
                                </div>
                                <h4 class="text-lg font-bold text-gray-900 dark:text-white">Shipping Information</h4>
                            </div>
                            <div class="space-y-2 text-gray-600 dark:text-gray-300">
                                <p class="font-medium text-gray-900 dark:text-white">
                                    <asp:Label ID="lblShippingName" runat="server" />
                                </p>
                                <p>
                                    <asp:Label ID="lblBillingAddress" runat="server" />
                                </p>
                                <p class="flex items-center">
                                    <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"/>
                                    </svg>
                                    <asp:Label ID="lblShippingEmail" runat="server" Text="Shipping Email"/>
                                </p>
                            </div>
                        </div>
                    </div>

                    <!-- Order Items Table -->
                    <div class="mb-8">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center">
                            <svg class="w-5 h-5 mr-2 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                            </svg>
                            Order Summary
                        </h3>
                        
                        <div class="overflow-x-auto">
                            <table class="w-full bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg">
                                <thead class="header-gradient text-white">
                                    <tr>
                                        <th class="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider">Product</th>
                                        <th class="px-6 py-4 text-right text-sm font-bold uppercase tracking-wider">Price</th>
                                        <th class="px-6 py-4 text-center text-sm font-bold uppercase tracking-wider">Quantity</th>
                                        <th class="px-6 py-4 text-right text-sm font-bold uppercase tracking-wider">Subtotal</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    <asp:Repeater ID="rptInvoiceItems" runat="server">
                                        <ItemTemplate>
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center">
                                                        <div class="w-2 h-2 bg-orange-500 rounded-full mr-3"></div>
                                                        <span class="text-sm font-medium text-gray-900 dark:text-white"><%# Eval("Product_Name") %></span>
                                                    </div>
                                                </td>
                                                <td class="px-6 py-4 text-right text-sm text-gray-900 dark:text-white font-medium">
                                                    R<%# Eval("Product_Price") %></td>
                                                <td class="px-6 py-4 text-center">
                                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                                                        <%# Eval("Quantity") %>
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 text-right text-sm font-bold text-gray-900 dark:text-white">
                                                    R<%# Eval("Subtotal") %></td>
                                            </tr>
                                        </ItemTemplate>
                                    </asp:Repeater>
                                </tbody>
                            </table>
                        </div>
                    </div>

                    <!-- Totals and Payment Summary -->
                    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end">
                        
                        <!-- Payment Method -->
                        <div class="mb-6 lg:mb-0">
                            <h4 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Payment Method</h4>
                            <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-200 dark:border-blue-800 max-w-sm">
                                <div class="flex items-center">
                                    <svg class="w-8 h-8 text-blue-500 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                    </svg>
                                    <div>
                                        <p class="font-semibold text-blue-800 dark:text-blue-300">Credit Card</p>
                                        <p class="text-sm text-blue-600 dark:text-blue-400">**** **** **** 4242</p>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <!-- Order Totals -->
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg min-w-80">
                            <h4 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Order Total</h4>
                            
                            <div class="space-y-3">
                                <div class="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700">
                                    <span class="text-gray-600 dark:text-gray-400">Subtotal:</span>
                                    <span class="font-medium text-gray-900 dark:text-white">
                                        R<asp:Label ID="lblSubtotal" runat="server" />
                                    </span>
                                </div>
                                
                                <div class="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700">
                                    <span class="text-gray-600 dark:text-gray-400">Shipping:</span>
                                    <span class="font-medium text-gray-900 dark:text-white">
                                        <asp:Label ID="lblShipping" runat="server" CssClass="font-medium text-green-600 dark:text-green-400"/>
                                    </span>
                                </div>
                                
                                <div class="flex justify-between items-center py-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg px-4 border-2 border-orange-200 dark:border-orange-800">
                                    <span class="text-lg font-bold text-orange-700 dark:text-orange-300">Total Paid:</span>
                                    <span class="text-xl font-bold text-orange-700 dark:text-orange-300">
                                        R<asp:Label ID="lblTotal" runat="server" />
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex flex-col sm:flex-row gap-4 mt-8 pt-8 border-t border-gray-200 dark:border-gray-700 no-print">
                        <button 
                            onclick="window.print()" 
                            class="print-animation flex items-center justify-center px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/>
                            </svg>
                            Print Invoice
                        </button>
                        
                        <asp:Button ID="btnInvoiceHistory" runat="server" 
                            Text="View Invoice History"
                            CssClass="flex items-center justify-center px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105" OnClick="btnInvoiceHistory_Click" />

                       <%-- <button 
                            onclick="downloadPDF()"
                            class="print-animation flex items-center justify-center px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                            </svg>
                            Download PDF
                        </button>--%>
                    </div>

                    <!-- Message Section -->
                    <div class="mt-6">
                        <h4 id="lblMessage" runat="server" class="text-lg font-medium text-gray-900 dark:text-white"></h4>
                    </div>
                </div>
            </div>

            <!-- Thank You Section -->
            <div class="mt-8 text-center no-print">
                <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg">
                    <div class="flex items-center justify-center mb-4">
                        <div class="w-16 h-16 bg-green-100 dark:bg-green-900/20 rounded-full flex items-center justify-center">
                            <svg class="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                            </svg>
                        </div>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Thank You for Your Purchase!</h3>
                    <p class="text-gray-600 dark:text-gray-300 max-w-md mx-auto">
                        We appreciate your business. Your order will be processed and shipped according to your selected delivery method.
                    </p>
                </div>
            </div>
        </div>
    </section>

    <script>
        // Set current date
        document.addEventListener('DOMContentLoaded', function() {
            const now = new Date();
            const dateString = now.toLocaleDateString('en-ZA', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
                minute: '2-digit'
            });
            document.getElementById('currentDate').textContent = dateString;
        });

        //function downloadPDF() {
        //    // PDF download functionality would be implemented here
        //    alert('PDF download functionality would be implemented here');
        //}

        // Add smooth entrance animations
        document.addEventListener('DOMContentLoaded', function() {
            const elements = document.querySelectorAll('.invoice-container > div');
            elements.forEach((element, index) => {
                element.style.opacity = '0';
                element.style.transform = 'translateY(20px)';
                
                setTimeout(() => {
                    element.style.transition = 'all 0.6s ease';
                    element.style.opacity = '1';
                    element.style.transform = 'translateY(0)';
                }, index * 100);
            });
        });
    </script>
</asp:Content>